...
const { TEXT_SEARCH_TERMS } = require("./config/search-terms.js");
...
function mapPlace(place, searchTerm, category) {
...
async function insertPlaces(pool, places, searchTerm, category) {
  let inserted = 0;
  let skipped = 0;
  for (const place of places) {
    const v = mapPlace(place, searchTerm, category);
    const result = await pool.query(INSERT_SQL, buildValues(v));
    if (result.rowCount > 0) {
      inserted++;
      console.log(`    ✓ ${v.name}`);
    } else {
      skipped++;
      console.log(`    – ${v.name} (duplicate)`);
    }
  }
...
async function runTextSearch(pool, entry, stats) {
  ...
  do {
    if (page > 0) {
      console.log(`  ↳ page ${page + 1} — waiting 2s…`);
      await sleep(2000);
    }
    page++;

    const body = { textQuery: term };
    ...
    const counts = await insertPlaces(pool, places, term, category);
...
async function main() {
  ...
  const stats = { apiCalls: 0, inserted: 0, skipped: 0 };

  try {
    // ── Phase 1: Text Search ──────────────────────────────────────────────────
    console.log("\n" + "═".repeat(60));
    console.log(`TEXT SEARCH — ${TEXT_SEARCH_TERMS.length} terms`);
    console.log("═".repeat(60));

    for (const entry of TEXT_SEARCH_TERMS) {
      console.log(`\n▸ "${entry.term}" [${entry.category}]`);
      try {
        const { inserted, skipped } = await runTextSearch(pool, entry, stats);
        stats.inserted += inserted;
        stats.skipped += skipped;
        console.log(
          `  → inserted: ${inserted}, skipped: ${skipped}  ` +
          `(running total: ${stats.inserted} inserted, ${stats.skipped} skipped)`
        );
      } catch (err) {
