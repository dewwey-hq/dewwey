...
- [`score-pass-1.md`](score-pass-1.md), [`score-pass-2.md`](score-pass-2.md), [`score-pass-3.md`](score-pass-3.md) — dated scorecard runs against the rubric (legacy `venue_enrichment` pipeline, retired by D060; kept for the rubric's pass history)
- [`loop/`](loop/README.md) — **the VenueDetails v3 fill loop (D060, 2026-09-19 →)**: tick protocol, gates, budget, recovery; `loop/ticks.md` is the append-only tick log, `loop/reports/<tick>/` what each tick printed
- [`golden-set-template.md`](golden-set-template.md) — the serving-side counterpart to the rubric above: canonical section order, pill/icon vocabulary, and archetype-conditional rules for the hand-built venue concept pages (`apps/web/app/concept/`)
