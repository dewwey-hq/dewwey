...
/**
 * Floor-plan resource button + in-site lightbox — same component as Marchetti's FloorPlanButton,
 * reused verbatim. Here it shows the venue's 2 real WEDDING-labeled floor plans specifically
 * (not the gala/corporate ones also on the venue's floor-plans page) — see data.ts header for
 * why that distinction is this venue's whole reason for being in the golden set.
 */
export function FloorPlanButton({ spaceName, floorPlans }: { spaceName: string; floorPlans: FloorPlan[] }) {
  ...
  return (
    <>
      ...
      >
        <FileText size={12} />
        Wedding floor plans ({floorPlans.length})
      </button>
