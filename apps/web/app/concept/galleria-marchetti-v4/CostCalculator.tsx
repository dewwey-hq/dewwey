...
export function CostCalculator() {
  ...
  const addOnOptions: AddOnOption[] = useMemo(() => {
    ...
    // Ceremony fee and Coat check (added 2026-08-26, sourced from the wedding brochure) flow
    // through this same generic loop automatically — Ceremony fee has a real dollar figure per
    // space ($1,000 / $2,000) so it just works. Coat check doesn't ("Available, fee varies"), so
    // it's filtered out below rather than silently pricing it at $0, which would misrepresent it
    // as free.
    const rentalOptions = marchetti.enhancements.flatMap((e) =>
      e.variants
        .filter((v) => /\$/.test(space.name === "La Pergola" ? v.pergola : v.pavilion))
        .map((v) => {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-black/[0.06]">
      ...
      {/* Financials — grouped into Venue and Food & Beverage subtotals, matching LondonHouse's
          ...
          service-charged (the 25% only applies to food & beverage).
          Sales tax is deliberately NOT applied to the flat venue rental fee (feedback 2026-08-26:
          "should the sales tax be on everything?") — Marchetti's own site only says "applicable
          sales tax will be added to all taxable charges," without saying room rental itself is
          one of them, and the 11.75% figure used here is Chicago's prepared-food/restaurant tax
          rate (see `additionalCosts.salesTaxPercent`'s own comment), not a general venue-rental
          tax. A straight room rental is typically not itself subject to that tax the way food,
          beverage, and rented goods are, so taxing it here would likely overstate the real
          number. Given its own line item ("Taxes" header below, stronger top border on the row
          itself) instead of trailing directly under whichever group happens to render above it
          (Food & beverage, or Space & rentals if any are selected) — feedback: it "looks
          grouped" with whatever's above rather than reading as its own thing. */}
        ...
        <table className="w-full text-sm text-gray-600">
          <tbody>
            ...
            <tr className="border-t border-black/[0.1]">
              <td className="py-1.5 pl-1">Sales tax ({marchetti.additionalCosts.salesTaxPercent}%, on food & beverage and rentals)</td>
