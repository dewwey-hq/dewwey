...
/**
 * The production feed card — promoted from the D058 lab (`app/lab/feed/variants/Card.tsx`,
 * variant "C · Card") after the user signed off on the design ("i love this"). One card =
 * one wedding: Instagram's official `embed.js` embed flush in the card's own left column
 * (never cropped, attribution intact), a stack panel on the right (building icon + venue
 * title, month, hairline, a pinned first tile, vendor tiles with derived display names and
 * category-icon avatars, a height-capped fold), a Caption/Photo pill that flips the photo
 * column to the caption at the same height, and — for multi-post weddings — a full-width
 * snap carousel with arrows outside the card and a dash/dots row under it. See the lab's
 * `Card.tsx` history for the day of design iteration this shape came out of.
 */

/** "July 2026" — no couple names anywhere in this card (user feedback, 2026-09-11: keep
 * the card, drop `titleFromCaption`). Wedding dates are date-only; read as UTC so a local
 * timezone can't push the month/year off by one (same rationale as `formatEventDate`). */
 ...
 * (`split`) can be any of the allowed percentages without a static class per combination:
 * the embed column is `embedWidth`, the stack column is derived from the share
 * (`embedWidth * (100 - split) / split`) because the embed itself can't grow past Meta's
 * 540px. Tailwind only needs the one arbitrary-property class below. */
...
/** Mobile-only fold cap. At md+ the stack panel shows every tile and scrolls internally
 * instead of folding -- the fold only still makes sense below md, where the panel is
 * stacked under the embed with no height to respect. */
const TILE_CAP = 6;

/** One tile in the stack grid — avatar, name (+ `@handle` when the name had to be derived
 * from it, large screens only), role/context subtitle, small `AddToTeamButton`. */
...
/** FLIP target for "Show caption" — replaces the embed+dots content in the media column
 * at the SAME height that column was last measured at, so toggling never resizes the
 * card. Owner row (avatar + `@handle`, linked to the post) up top (the Photo pill in the
 * action row flips back), caption text scrolling within the fixed height below. */
...
/**
 * One wedding, rendered as a card: a compliant `embed.js` embed flush against the card's
 * own edges (so the card's rounded corners clip the embed's own white border, nothing
 * else) beside a stack panel — "Hosted at {venue}" header, muted meta line, a hairline
 ...
 * height and scrolls internally with a bottom fade while there's more to scroll to;
 * below md there's no side-by-side height to respect, so it keeps the old 6-tile fold.
 */
export function WeddingCard({
  ...
  stack: WeddingStack;
  /** The account to pin as the stack panel's first tile (case-insensitive match against
   * `stack.vendors[].username`) — e.g. the vendor page's own account, so a photographer's
   * feed leads every card with the photographer. Falls back to the venue
   * (`stack.venue_username`) when omitted. The venue TITLE row (building icon + venue
   * name + month) is unaffected either way. */
  ...
  // Carousel track -- one full-width slide per embeddable post. Mount-only jump to the
  // cover slide (no smooth scroll, so it never fights a later programmatic/manual
  // scroll); after that, `goToPost`'s smooth `scrollTo` and native swipes both land on
  // this same rAF-debounced scroll listener, which derives `idx` from `scrollLeft` so the
  // pager's bars stay in sync either way.
  ...
  // loaded frame -- without a floor the panel cap collapsed and sprang back, which read
  // as "the stack reloads". While a switch is in flight the cap never drops below the
  // previous height; the floor clears once the column has been still for 3 s.
  ...
  // The pinned tile: `pinnedUsername` when given and present on this stack (e.g. the
  // vendor page's own account), else the venue -- same default as before pinning existed.
  const pinnedKey = pinnedUsername?.toLowerCase();
  const pinnedVendor = pinnedKey
    ? stack.vendors.find((v) => v.username.toLowerCase() === pinnedKey)
    : venueVendor;
  const others: StackVendor[] = pinnedVendor ? stack.vendors.filter((v) => v !== pinnedVendor) : stack.vendors;
  const groups = groupStackByCategory(others);
  // The pinned account is the FIRST tile, then everyone else by category.
  const tiles = [
    ...(pinnedVendor ? [{ ...pinnedVendor, extraRoles: [] as string[] }] : []),
    ...groups.flatMap((g) => g.vendors),
  ];
  // Fold: the cap is however many tiles fit beside the embed -- derived from the measured
  // media height minus the panel's header/divider/button chrome (~150px) at ~44px per
  // tile -- so the folded panel never needs to scroll. Expanded, the panel still can't
  // outgrow the embed; it scrolls with the scrollbar hidden (`scrollbar-none`) and the
  // fade below as the only affordance.
  ...
  // `stack.venue_name` is already the per-wedding fallback straight from the query.
  const venueLabel = venueVendor ? displayName(venueVendor.name, venueVendor.username) : (stack.venue_name ?? "Unknown venue");

  // Side by side at md+ in BOTH layouts. 1-up: fixed px columns (embed at `embedWidth`,
  // stack derived from `split`). 2-up: the cell is ~half the page, so fractional columns
  // in the same ratio with the embed's 326px floor; the embed fills its column (≤540).
  // `mediaSide` flips the column order (and the template) — the card's overflow-hidden
  // rounds whichever corners the embed ends up on.
  ...
  return (
    ...
    {/* Post switcher: round arrows OUTSIDE the card, flanking it at md+ (hidden on phone,
        where the track swipes), and a dash-for-active / dots-for-the-rest row centered
        under the card. Nothing under the embed inside the card. */}
    {n > 1 && (
      <>
        <button
          type="button"
    ...
    >
      {/* Media -- flush against the card's own edges (no padding) so the card's rounded
          ...
          else drawn over it. Dots pager directly under the embed. */}
      <div ref={mediaRef} className={`flex flex-col ${mediaBorderClass}`}>
        ...
        <div ref={swapRef} hidden={captioned}>
          {/* Snap track -- one full-width slide per embeddable post, swipeable natively on
              touch. `overflow-x-hidden` on the outer wrapper is a guard so the track's own
              horizontal scroll never leaks into the page; `items-start` on the non-wrapping
              flex row is what makes the row's height track the tallest loaded slide. Only
              the cover slide is `eager` -- every other slide relies on `InstagramPostEmbed`'s
              own IntersectionObserver lazy mount, which fires naturally as it's scrolled
              into view. embed.js puts an inline `margin: 0 0 12px` on the iframe it injects
              -- zero it here, scoped to this column only. */}
      ...
      {/* Stack panel -- white (no grey), "Hosted at" header + meta line + tile grid, venue
          excluded (already the Hosted line) unless it's the pinned tile. Capped to the
          media column's height at md+ (`--media-h`) and scrolls internally instead of
          growing the card; below md it's the old 6-tile fold with no cap. */}
      <div
        ref={panelRef}
      ...
      >
        <div className="flex flex-col p-5">
          {/* Building icon so "venue" isn't undersold: the venue name IS the panel title;
              one subtitle line. The only action is the caption/photo flip, icon-only,
              top-right (no "Open" -- the embed's own "View more on Instagram" covers it). */}
          ...
          {/* Hairline separating the header/meta block from the vendor stack. */}
        ...
        {/* Bottom scroll affordance -- only while the panel is actually capped and there's
            more below the fold. `-mt-8` pulls it over the last 32px of content instead of
            adding scroll height of its own; `sticky bottom-0` keeps it pinned while
            scrolling. */}
        {canScrollMore && (
          <div
            aria-hidden
