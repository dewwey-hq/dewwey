/**
 * Builds the both-sides review artifact for the 268 ambiguous
 * jeremy_wedding_candidate_reconciliation matches (reconcile-v2,
 * matched_wedding_id set, match_confidence NOT in the 0.75–0.85 high
 * bucket that D020 already audited). Mirror of exportHighConfidenceReview.ts
 * with only the tier filter changed.
 *
 * Ben-side source posts: wedding_posts(wedding_id, post_id) -> posts.url —
 * the same join D020 used.
 *
 ...
 *
 * Usage (from apps/web): bun run scripts/graph/exportAmbiguousReview.ts
 */
...
async function main() {
  ...
  const { rows } = await pool.query<{
    ...
    `select r.candidate_id, r.matched_wedding_id, va.username as venue_handle,
     ...
     where r.reconciliation_version = $1
       and r.matched_wedding_id is not null
       and not (r.match_confidence between 0.75 and 0.85)
     order by r.candidate_id`,
    [RECONCILIATION_VERSION]
  );

  console.log(`[export-review] ${rows.length} ambiguous matches to build`);
  ...
  const csvPath = "scripts/graph/data/reconciliation_review/ambiguous_268.csv";
  ...
  const mdPath = "scripts/graph/data/reconciliation_review/ambiguous_268.md";
